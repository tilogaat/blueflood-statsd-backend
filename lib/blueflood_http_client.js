  ...
  var request = transport.request(options, function(response) {
    response.on('data', function(data) {
      if (response.statusCode == 401) {
        log.log(response.statusCode + ": Stale auth token");
        var reauth_callback = function (token) {
          if(token != null || typeof token != 'undefined') {
            ...
            settTimeout(postRaw.bind(null, options, transport, payload, log, retry + 1), RETRY_DELAY_SECS * 1000);
          }
          else {
            log.log(response.statusCode + ' No token received! Error!');
          }
        }
        authenticate("tenantid","89ecb97253191d56c1689c0d019c55b6","corpdev0paxon0",reauth_callback);
      }
...
function authenticate(tenantid,apikey,username,callback) {
  token = null;
